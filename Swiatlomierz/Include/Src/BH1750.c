 ...
 */
short begin(BH1750* bh1750,Mode mode, uint8_t addr, TwoWire* i2c) {
...
short configure(BH1750* bh1750,Mode mode) {
  ...
  case ONE_TIME_LOW_RES_MODE:
    ...
    // Send mode to sensor
     i2c_start(BH1750_I2CADDR);
    i2c_byte((uint8_t)mode);
    ack = i2c_readAck();
 ...
 * @return short true if MTReg successful set
 ...
 * @param maxWait a shortean if to wait for typical or maximum delay
 * @return a shortean if a new measurement is possible
 *
 */
short measurementReady(BH1750* bh1750,short maxWait) {
